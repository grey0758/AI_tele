...
) -> Tuple[List[InputDevice], List[OutputDevice]]:
    """
    获取所有音频设备信息
...
def find_device_by_name(device_name: str, device_type: str = "both") -> Tuple[int, int]:
    ...
    if device_type in ["input", "both"]:
        for input_device in input_devices:
            if device_name in input_device.name:
                input_index = input_device.index
                break

    if device_type in ["output", "both"]:
        for output_device in output_devices:
            if device_name in output_device.name:
                output_index = output_device.index
                break
...
if __name__ == "__main__":
    ...
    # 获取去重后的设备
    input_devs, output_devs = get_audio_devices(deduplicate=True)
